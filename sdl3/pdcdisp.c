...
static Uint32 get_pdc_mapped( const int color_idx)
{
   ...
   return(SDL_MapSurfaceRGB( pdc_screen, c->r, c->g, c->b));
}
...
static void _set_attr(chtype ch)
{
    ...
    if (oldch != ch)
    {
        ...
        {
#ifndef PDC_WIDE
            SDL_Palette *palette = SDL_GetSurfacePalette(pdc_font);
            SDL_SetPaletteColors(palette, get_pdc_color(newfg), pdc_flastc, 1);
#endif
            foregr = newfg;
        }
        ...
        if (newbg != backgr)
        {
            ...
            else
            {
                ...
                SDL_Palette *palette = SDL_GetSurfacePalette(pdc_font);
                SDL_SetPaletteColors(palette, get_pdc_color( newbg), 0, 1);
            }
#endif
            backgr = newbg;
        }
...
static bool _grprint(chtype ch, const SDL_Rect dest)
{
    ...
    if( remap_tbl[i] == (int)ch)
    {
        ...
        if( ch == ACS_BLOCK)
            SDL_FillSurfaceRect(pdc_screen, &temp, col);
        if( mask & HORIZ)
        {
            temp.h = pdc_fthick;
...
#ifdef PDC_WIDE
    SDL_FillSurfaceRect(pdc_screen, &dest, get_pdc_mapped( backgr));
...
void _new_packet(attr_t attr, int lineno, int x, int len, const chtype *srcp)
{
    ...
    for (j = 0; j < len; j++)
    {
        ...
        if (!blink && (attr & (A_LEFT | A_RIGHT)))
        {
            ...
            if (attr & A_LEFT)
                SDL_FillSurfaceRect(pdc_screen, &dest, get_pdc_mapped( hcol));
...
static Uint32 _blink_timer(void *param, SDL_TimerID timerID, Uint32 interval)
{
    SDL_Event event;

    INTENTIONALLY_UNUSED_PARAMETER( param);
    INTENTIONALLY_UNUSED_PARAMETER(timerID);
...
{
    SDL_Event win_events[8];
    int count, i;

    /* Pump OS events into the SDL queue without removing anything. */
    SDL_PumpEvents();

    /* Extract all window events (0x202-0x23F range) without disturbing
       keyboard/text events (0x300+ range), which must stay in order. */
    count = SDL_PeepEvents(win_events, 8, SDL_GETEVENT,
                           SDL_EVENT_WINDOW_FIRST, SDL_EVENT_WINDOW_LAST);
    for (i = 0; i < count; i++)
    {
        if (win_events[i].type == SDL_EVENT_WINDOW_RESTORED ||
            win_events[i].type == SDL_EVENT_WINDOW_EXPOSED ||
            win_events[i].type == SDL_EVENT_WINDOW_SHOWN)
        {
            SDL_UpdateWindowSurface(pdc_window);
            rectcount = 0;
        }
        else
            SDL_PushEvent(&win_events[i]);  /* e.g. RESIZED, handled in pdckbd.c */
    }
}
