...
/* keycode      keypad  normal       shifted       control      alt*/
 ...
 {SDLK_MEDIA_NEXT_TRACK, FALSE, KEY_NEXT_TRACK, KEY_NEXT_TRACK, KEY_NEXT_TRACK, KEY_NEXT_TRACK },
 {SDLK_MEDIA_PREVIOUS_TRACK, FALSE, KEY_PREV_TRACK, KEY_PREV_TRACK, KEY_PREV_TRACK, KEY_PREV_TRACK },
 {SDLK_MEDIA_STOP, FALSE, KEY_MEDIA_STOP, KEY_MEDIA_STOP, KEY_MEDIA_STOP, KEY_MEDIA_STOP },
 {SDLK_MEDIA_PLAY, FALSE, KEY_PLAY_PAUSE, KEY_PLAY_PAUSE, KEY_PLAY_PAUSE, KEY_PLAY_PAUSE },
 {0,            0,      0,           0,            0,           0}
};
...
static Uint64 _stored_timestamp;  /* SDL_GetTicks() ms, not event timestamp ns */

/* SDL3 event.text.text is a const char * into SDL-internal memory that may be
   freed by any subsequent SDL_PollEvent call (even into a local variable).
   We copy it here so we can safely walk multi-char IME sequences and avoid
   the stale-pointer hang in PDC_flushinp. */
static char _text_buf[64];
static int  _text_pos;
...
bool PDC_check_key(void)
{
    ...
    /* Walk remaining bytes from a multi-char IME TEXT_INPUT before polling. */
    if (_text_buf[_text_pos])
        return TRUE;

    haveevent = (bool)SDL_PollEvent(&event);
    if (haveevent && event.type == SDL_EVENT_TEXT_INPUT)
    {
        SDL_strlcpy(_text_buf, event.text.text, sizeof(_text_buf));
        _text_pos = 0;
    }
    return haveevent;
}
...
#ifdef PDC_WIDE
static int _utf8_to_unicode(const char *chstr, size_t *b)
{
    int i, bytes, unicode;
...
static int _process_key_event(void)
{
    ...
    else if (event.type == SDL_EVENT_TEXT_INPUT)
    {
...
#ifdef PDC_WIDE
        key = _utf8_to_unicode(_text_buf + _text_pos, &bytes);
        _text_pos += (int)bytes;
        rval = _handle_alt_keys(key);
#else
        key = (unsigned char)_text_buf[_text_pos++];
        rval = (key > 0x7f ? -1 : _handle_alt_keys(key));
#endif
        if (!_text_buf[_text_pos])
        {
            /* All text consumed — clear so PDC_check_key won't see it again. */
            _text_buf[0] = '\0';
            _text_pos = 0;
            event.type = 0;
        }
        ...
        {                           /* etc.  Wait 2 ms to see if a PADx   */
            ...
            _stored_timestamp = SDL_GetTicks();   /* milliseconds */
            rval = -1;
        }
...
static int _process_mouse_event(void)
{
    ...
    {
        short action = (event.button.down == true) ?
        ...
        if (action == BUTTON_PRESSED && SP->mouse_wait)
        {
            ...
            {
                if (rel.type == SDL_EVENT_MOUSE_BUTTON_UP && rel.button.button == btn)
                {
                ...
                else if(rel.type != SDL_EVENT_MOUSE_BUTTON_DOWN || rel.button.button != btn)
                {
                    SDL_PushEvent(&rel);
                    break;
                }
